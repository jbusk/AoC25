...
foreach (var b1 in boxes)
{
    foreach (var b2 in boxes)
    {
        ...
        if (distances.ContainsKey((b1, b2)) || distances.ContainsKey((b2, b1)))
            continue;
        distances.Add((b1, b2), Math.Sqrt(Math.Pow(b1.x - b2.x, 2) + Math.Pow(b1.y - b2.y, 2) + Math.Pow(b1.z - b2.z, 2)));
    }
}
...
foreach (var pair in connections)
{
    ...
    foreach (var b1 in circuit)
        circuits[b1] = circuit;
    if (circuits.Count == lines.Length)
    {
        Console.WriteLine("Part 2: " + pair.Item1.x * pair.Item2.x);
        break;
    }
    ...
    if (i == iterationsPt1)
    {
        var sizes = circuits.Values
            ...
            .Distinct()
            .Take(3);
        Console.WriteLine("Part 1: " + sizes.Aggregate(1, (a, b) => a * b));
    }
}